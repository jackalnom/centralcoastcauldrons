...
VALUES 
('LARGE_RED_BARREL', 100, 'red'::text, ARRAY[100, 0, 0], 50, 10),
('MEDIUM_RED_BARREL', 25, 'red'::text, ARRAY[100, 0, 0], 25, 10),
('SMALL_RED_BARREL', 5, 'red'::text, ARRAY[100, 0, 0], 10, 10);
...
BEGIN
  ...
  -- Purchase barrels by deducting money and adding barrels
    ...
    -- FIX UP THIS FOR NEW BARREL TYPE
  ...
  BEGIN
    ...
    SELECT json_agg(barrels) into barrel_catalog from 
      (select sku, liters_per_barrel, color, price, quantity from
        (
          SELECT barrel_catalog_items.sku, barrel_catalog_items.liters_per_barrel, barrel_catalog_items.color, barrel_catalog_items.price, barrel_purchases.quantity
          FROM barrel_purchases
          JOIN barrel_catalog_items ON barrel_purchases.sku = barrel_catalog_items.sku
          WHERE barrel_purchases.shop_id = purchase_barrels.shop_id AND barrel_purchases.tick_id = purchase_barrels.tick_id
        ) sub
